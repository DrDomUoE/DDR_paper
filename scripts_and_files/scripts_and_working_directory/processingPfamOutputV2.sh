...
#run this script 2nd after you have run  convertingTSVtoSED_unixVersionV3.sh
...
sed -f sed_file_domainology.txt < ortholog_table.tsv > ortholog_table_domain_lengths.tsv
...
./convert_final_sed_filetsv_inProcessingPfamSh.sh final_sed_file.tsv tmp2.tsv
...
sed -f sed_file.txt < list_of_proteins_from_ortholog_table.tsv > temp_full_length.txt
...
sed -f sed_file_interdomain_length.txt < ortholog_table.tsv > ortholog_table_interdomain_lengths.tsv
rm temp_concat*
...
./convert_final_sed_filetsv_inProcessingFfamSh.sh final_sed_file_interdomain_lengths.tsv tmp2.tsv
...
sed -f sed_file.txt < tmp1.txt > tmp2.txt
...
tr -d '\n' < temp_concat6.txt > temp_concat7.txt
sed 's/£/\n/g' < temp_concat7.txt > temp_concat8.txt
sed 's/$/#/g' < temp_concat8.txt > temp_concat9.txt
sed 's/^>/#>/g' < temp_concat9.txt > sed_file_full_protein_length.txt

echo ">>>Step 9.2: Creating ortholog_table_full_protein_lengths.tsv"
sed -f sed_file_full_protein_length.txt < ortholog_table.tsv > ortholog_table_full_protein_lengths.tsv
rm temp_concat*
...
./convert_final_sed_filetsv_inProcessingFfamSh.sh final_sed_file_full_protein_lengths.tsv tmp2.tsv
...
#rm tmp*.tsv
rm column*.txt
echo ">>>Step 9.3: Creating ortholog_table_domain_count.tsv"
./count_pfam.sh hmmer_pfam_output_tbl_trimmed.txt sed_file_hmmer_pfam_output_tbl_trimmed_domain_count.txt
sed -f sed_file_hmmer_pfam_output_tbl_trimmed_domain_count.txt < ortholog_table.tsv > ortholog_table_domain_count.tsv
echo ">>>Step 9.4: Creating ortholog_table_domain_count_averages.tsv"
./calculating_domain_count_averages.sh ortholog_table_domain_count.tsv > tmp.tsv
tr ' ' '\t' < tmp.tsv > ortholog_table_domain_count_averages.tsv
echo ">>>done. thank you for waiting so patiently<<<"
